...
public class VideoCaptureProcessor implements Observer {
		...
		captureProcessorFrame.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				if (threadRunning) {
					...
					if (JOptionPane.showOptionDialog(captureProcessorFrame, "A thread is running in the background. "
							...
							+ "after the action has completed.", "Thread Running", JOptionPane.YES_NO_OPTION,
	...
	private boolean keepFile(String oldFilename, File destDir, String newFilename) {
		...
		try {
			for (File srcFile : fileList) {
				int result = FileUtility.moveFiles(srcFile, destDir,
	...
	private void markClip() {
		...
		try {
			FileUtility.writeStringToFile(clipTime + clipLabel,
					new File(CLIPS_DIRECTORY + filename + CLIPS_FILE_EXTENSION), true);
	...
	public static void setFileChooserLastPath(File path) throws IOException {
		...
		public void run() {
			...
			if (fileList != null && fileList.length != 0) {
				...
				int choice = JOptionPane.showOptionDialog(captureProcessorFrame,
						...
						"Clips Exist", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE, null, options,
						options[2]);
			...
			int undeleted = FileUtility.deleteFilesFromDirectory(new File(
					propertiesInstance.getProperty(CaptureProcessorProperties.CAPTURE_LOCATION_PROPERTY.getName())));
